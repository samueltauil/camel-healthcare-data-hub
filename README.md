...
### 4. Seed the FTP server with Synthea data

```bash
./scripts/seed-ftp.sh
```

This uploads the Synthea-generated files (CSV + HL7) from `sample-data/ftp-seed/` to the FTP `inbox/` directory. Data flows through the pipeline only when files land in the FTP inbox — once ingested, each file is parsed and fanned out to all output connectors in parallel.

## Testing the Output Connectors

Start the full stack, run the app, and seed data to trigger the pipeline:

```bash
docker-compose up -d
mvn clean quarkus:dev

# Generate and upload Synthea data
./scripts/generate-synthea-data.sh
./scripts/seed-ftp.sh
```
...
```bash
# Synthea generates HL7 files — they are forwarded via MLLP automatically
# when uploaded via seed-ftp.sh. Watch the MLLP receiver output:
...
All sample data is generated by [Synthea](https://github.com/synthetichealth/synthea) — a realistic synthetic patient generator used widely in healthcare IT.

```bash
# Generate 20 patients (default)
./scripts/generate-synthea-data.sh

# Generate 100 patients in Texas
./scripts/generate-synthea-data.sh 100 Texas
```

### Generated outputs (gitignored, created on demand)
...
- `sample-data/ftp-seed/` — Pre-selected files ready for FTP upload via `seed-ftp.sh`
